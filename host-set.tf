...
will look for to automatically pull into Boundary as resources to access. The preferred_endpoints addresses need to be changed to use string
...
resource "boundary_host_set_plugin" "azure_example" {
  name                  = "Azure host set plugin"
  host_catalog_id       = boundary_host_catalog_plugin.azure_plugin.id
  preferred_endpoints   = ["cidr:0.0.0.0/0"]
  sync_interval_seconds = 30
